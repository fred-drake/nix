  ...
  services = {
    openssh = {
      ...
      settings = {
        ...
        PermitRootLogin = "no";
        ListenAddress = config.soft-secrets.host.sonarqube.admin_ip_address;
      };
    };
  };
  networking.hostName = "sonarqube";
  users.users.default = {
    isNormalUser = true;
    password = "";
    extraGroups = ["wheel"];
  ...
  networking = {
    ...
    interfaces."eth0" = {
      ...
      ipv4 = {
        addresses = [
          {
            address = config.soft-secrets.host.sonarqube.admin_ip_address;
            prefixLength = 24;
          }
        ];
      };
    ...
    interfaces."eth0.50".ipv4 = {
      addresses = [
        {
          address = config.soft-secrets.host.sonarqube.service_ip_address;
          prefixLength = 24;
        }
      ];
      routes = [
