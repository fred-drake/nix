...
in {
  systemd.tmpfiles.rules = [
    "d /var/calibre-web/config 0755 99 100 -"
  ];

  security.acme.certs."calibre-web.${config.soft-secrets.networking.domain}" = {
  ...
  services.nginx.virtualHosts."calibre-web.${config.soft-secrets.networking.domain}" = {
    enableACME = true;
    forceSSL = true;
    locations."/" = {
