export const meta = {
  ...
  phases: [
    { title: 'Pre-flight', detail: 'repo sanity + host reachability', model: 'sonnet' },
    { title: 'Workaround Audit', detail: 'retire stale WORKAROUND overrides against the pinned unstable rev' },
    { title: 'Deploy', detail: 'colmena apply, one host at a time, in canonical order', model: 'sonnet' },
    { title: 'Verify', detail: 'curl every fleet web site after each switch', model: 'sonnet' },
    { title: 'Fix', detail: 'diagnose/fix a failed switch, then restart from stormwind' },
    { title: 'Heal', detail: 'fix unhealthy sites before advancing to the next host' },
  ],
}
...
const ENDPOINTS_DOC = `${REPO}/.claude/skills/infrastructure/references/host-mapping.md`
...
${REPO}/.claude/skills/infrastructure/SKILL.md — read it first.
...
- Config/eval/build bugs → fix the nix code in ${REPO}. Repo conventions: temporary overlay
  ...
  ${REPO}/.claude/skills/infrastructure/references/gnomeregan.md before touching gnomeregan's config.
...
- If the root cause is a nix config bug, fix it in ${REPO} (conventions: WORKAROUND markers for
  ...
  ${REPO}/.claude/skills/infrastructure/references/gnomeregan.md before touching gnomeregan's
...
async function runDeploy(host, label) {
  const d = await agent(deployPrompt(host), {
    label, phase: 'Deploy', schema: DEPLOY_SCHEMA, model: 'sonnet',
  })
...
async function fixSwitchFailure(host, deploy) {
  ...
  const fix = await agent(switchFixPrompt(host, deploy), {
    label: `fix-switch:${host}`, phase: 'Fix', schema: FIX_SCHEMA,
  })
...
Return raw data only.`,
  { label: 'pre-flight', phase: 'Pre-flight', schema: PREFLIGHT_SCHEMA, model: 'sonnet' }
)
...
if (!auditBuilder) {
  ...
  const audit = await agent(auditPrompt(auditBuilder), {
    label: 'workaround-audit', phase: 'Workaround Audit', schema: AUDIT_SCHEMA,
  })
  if (!audit) {
...
while (i < ORDER.length) {
  ...
  for (let round = 0; round <= MAX_HEAL_ROUNDS; ) {
    const probe = await agent(verifyPrompt(host, [...skipped]), {
      label: `verify:${host}#${round + 1}`, phase: 'Verify', schema: PROBE_SCHEMA, model: 'sonnet',
    })
    ...
    const heal = await agent(healPrompt(host, failing, probe.notes), {
      label: `heal:${host}#${round + 1}`, phase: 'Heal', schema: HEAL_SCHEMA,
    })
