export const meta = {
  ...
  phases: [
    { title: 'Validate', detail: 'detect project type and run all quality checks', model: 'opus' },
    { title: 'Fix', detail: 'fix failed checks, then re-validate (max 10 iterations)', model: 'opus' },
    { title: 'Commit', detail: 'stage safe files and create semantic commits', model: 'opus' },
    { title: 'Push', detail: 'push to origin and verify', model: 'opus' },
  ],
}
...
script), treat it as passed and note that in errors.`,
    { label: `validate#${i}`, phase: 'Validate', schema: VALIDATION_SCHEMA, model: 'opus' }
  )
...
suite will be re-run separately afterward. Report each fix you applied.`,
    { label: `fix#${i}`, phase: 'Fix', schema: FIX_SCHEMA, model: 'opus' }
  )
}
...
messages created, the files staged, and the files skipped with reasons.`,
  { label: 'stage-and-commit', phase: 'Commit', schema: COMMIT_SCHEMA, model: 'opus' }
)
...
4. Report the branch, remote URL, and a PR-creation link if the push output
   provides one.`,
  { label: 'push', phase: 'Push', schema: PUSH_SCHEMA, model: 'opus' }
)
